...
1. 把本仓库推送到 Cloud Studio 支持导入的代码仓库（CNB cnb.cool / GitHub / 工蜂）。
2. 在 Cloud Studio 工作台「从 Git 仓库导入」，粘贴仓库地址。
3. 打开内置终端，运行：
   ```bash
   python3 serve.py
   ```
   Cloud Studio 会自动探测端口并给出预览地址（默认 8080）。
4. 点击预览地址即可在浏览器中使用转换工具。

## 说明

- WASM 引擎首次加载约 6MB，需 5–10 秒编译，请耐心等待「WASM 引擎已就绪」提示。
- 适用于大文件，无服务器端大小限制。
- `serve.py` 已为 `.wasm` 显式设置 `application/wasm`，确保 Cloud Studio 预览环境能正确实例化 WASM。
